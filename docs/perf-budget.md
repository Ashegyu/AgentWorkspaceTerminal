...
|---|---|---|
| 키 입력 → 화면 echo p95 ≤ 50ms | ✅ (수동, App.Wpf 인스트루먼트) | `App.Wpf` 팔레트의 "Dump Echo Latency Samples…" 명령 — bridge.js가 실시간 키→렌더 round-trip을 ring buffer (last 500)에 누적, palette 호출 시 `awt-perfprobe echo-latency`로 자동 pipe되고 status bar에 p95 표시 |
...
| 4-pane idle RSS ≤ 500MB (full-stack: WPF + WebView2) | △ (수동, 1회/회귀) | `awt-perfprobe rss-full --pid <App.Wpf PID>` |
...
`rss-full`은 App.Wpf가 이미 떠 있는 상태(예: 4-pane 구성)에서 사람이 호출:

```pwsh
# App.Wpf 띄우고 4 pane 띄운 뒤
$pid = (Get-Process AgentWorkspace.App.Wpf).Id
& src/AgentWorkspace.PerfProbe/bin/Release/net10.0-windows/awt-perfprobe.exe rss-full --pid $pid --warmup-sec 3 --sample-sec 5
```

결과를 사람이 봐서 baseline.json `fourPaneIdleRssFullMb`을 채운다 (CI는 PID를 가질 수 없어 자동화 안 됨 — 명시적 update만 받는다).
